 ...
 *                connected on MIAT_STM32 evaluation board 
 * Version: V1.10
 *----------------------------------------------------------------------------
...
#define PIN_RW                (   1 << 6)
#define PIN_RS                (   1 << 7)
#define PINS_DATA             (0x0F <<  8)

/* Enable Clock for peripheral driving LCD pins                               */
#define LCD_CLOCK_EN         (RCC->APB2ENR |= (3 << 6)); // enable clock for GPIOC

/* pin E  setting to 0 or 1                                                   */
#define LCD_E(x)              GPIOE->ODR = (GPIOE->ODR & ~PIN_E)  | (x ? PIN_E : 0);

/* pin RW setting to 0 or 1                                                   */
#define LCD_RW(x)             GPIOF->ODR = (GPIOF->ODR & ~PIN_RW) | (x ? PIN_RW : 0);

/* pin RS setting to 0 or 1                                                   */
#define LCD_RS(x)             GPIOF->ODR = (GPIOF->ODR & ~PIN_RS) | (x ? PIN_RS : 0);

/* Reading DATA pins                                                          */
#define LCD_DATA_IN           (((GPIOF->IDR & PINS_DATA) >> 8) & 0x0F)

/* Writing value to DATA pins                                                 */
#define LCD_DATA_OUT(x)       GPIOF->ODR = (GPIOF->ODR & ~PINS_DATA) | ((x & 0x0F) << 8);

/* Setting all pins to output mode                                            */
#define LCD_ALL_DIR_OUT       GPIOE->CRL = (GPIOE->CRL & 0xF0FFFFFF) | 0x03000000; \
                              GPIOF->CRL = (GPIOF->CRL & 0x00FFFFFF) | 0x33000000; \
                              GPIOF->CRH = (GPIOF->CRH & 0xFFFF0000) | 0x00003333; 
 
/* Setting DATA pins to input mode                                            */
#define LCD_DATA_DIR_IN       GPIOF->CRH = (GPIOF->CRH & 0xFFFF0000) | 0x00004444;

/* Setting DATA pins to output mode                                           */
#define LCD_DATA_DIR_OUT      GPIOF->CRH = (GPIOF->CRH & 0xFFFF0000) | 0x00003333;
...
void FormatDec (unsigned char *pstr, unsigned int value, unsigned char digits)
{
    unsigned char  i;
    unsigned int   mult;
    unsigned char  found;
